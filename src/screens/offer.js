...
class offer extends Component {
    ...
    return [...Array(5)].map((el, i) =>
      ...
      i < num ? (
        <FontAwesome5
          ...
          name="star"
          solid
          style={{ color: "#e5cc25", margin: 1, fontSize: size }}
        />
      ) : (
        ...
        <FontAwesome5
          ...
          name="star"
          solid
          style={{ color: "#c4c4c4", margin: 1, fontSize: size }}
        />
      )
    );
  }
      ...
      if (i <= this.state.selectedRating) {
        React_Native_Rating_Bar.push(
          <TouchableOpacity
            key={i}
            onPress={() => {
              this.setState({ selectedRating: i });
            }}
          >
            <FontAwesome5
              key={i}
              name="star"
              solid
              style={{ color: "#e5cc25", margin: 1, fontSize: 24 }}
            />
          </TouchableOpacity>
        );
      } else {
        React_Native_Rating_Bar.push(
          ...
          >
            <FontAwesome5
              ...
              name="star"
              solid
              style={{ color: "#c4c4c4", margin: 1, fontSize: 24 }}
            />
          </TouchableOpacity>
        );
      }
  ...
  _openModal(type) {
    if (type == "viewRatings") {
      Animated.spring(this.state.viewRatingAnimate, {
        toValue: 0,
        velocity: 3,
        tension: 2,
        friction: 8
      ...
      Animated.spring(this.state.addRatingAnimate, {
        toValue: 0,
        velocity: 3,
        tension: 2,
        friction: 8
      ...
      Animated.spring(this.state.addOfferAnimate, {
        toValue: 0,
        velocity: 3,
        tension: 2,
        friction: 8
      ...
      Animated.spring(this.state.reportOfferAnimate, {
        toValue: 0,
        velocity: 3,
        tension: 2,
        friction: 8
  ...
  async getoffer() {
    ...
    try {
      await fetch(Constants.siteUrl + "offer/" + offerId, {
        ...
        .then(res => {
          if (res.status == 200) {
            this.setState({
              ...
              ratingCount: res.data.myrate.count,
              ratingStars: res.data.myrate.rate,
              rates: res.data.rates,
              // options: res.data.options[Object.keys(res.data.options)[0]].fields,
  ...
  async addOrder() {
    ...
    try {
      ...
      var sendArray = {};
      sendArray["id"] = offerId;
      sendArray["type"] = "offer";
      if (this.state.username != "") {
        sendArray["username"] = this.state.username;
      }
      if (this.state.email != "") {
      ...
      if (this.state.extraNotes != "") {
        sendArray["content"] = this.state.extraNotes;
      }
      await fetch(Constants.siteUrl + "orders", {
        ...
        .then(res => {
          ...
          if (Object.entries(res.errors).length > 0) {
            ...
            if (res.errors.hasOwnProperty("content")) {
              this.setState({ input4: "#ff0000", input4Icon: "#ff0000" });
              errors.content = res.errors.content;
            }
  ...
  async addRating() {
    try {
      ...
      const { navigation } = this.props;
      const offerId = navigation.getParam("offerId");
      const accountApiCall = await fetch(
        Constants.siteUrl + "offer/" + offerId + "/rates",
        {
          method: "POST", // *GET, POST, PUT, DELETE, etc.
          mode: "cors", // no-cors, cors, *same-origin
          cache: "no-cache", // *default, no-cache, reload, force-cache, only-if-cached
  ...
  async reportOffer() {
    ...
    try {
      ...
      await fetch(Constants.siteUrl + "offer/" + offerId + "/report", {
        method: "POST", // *GET, POST, PUT, DELETE, etc.
        mode: "cors", // no-cors, cors, *same-origin
        cache: "no-cache", // *default, no-cache, reload, force-cache, only-if-cached
        credentials: "same-origin", // include, *same-origin, omit
  ...
  renderOptions() {
    ...
    var lat = Object.entries(this.state.options).filter(e => {
      if (e[0] == "lat") {
        console.log("yes lat");
        return e[1];
      }
    ...
    for (const key in this.state.options) {
      if (this.state.options.hasOwnProperty(key) && key == "lat") {
        lat = this.state.options[key];
      }
      if (this.state.options.hasOwnProperty(key) && key == "lng") {
        lng = this.state.options[key];
      }
    }
      ...
      if (item[0] == "website") {
        React_Native_Rating_Bar.push(
          <View key={item.id} style={styles.itemHolder}>
            <Image
              source={require("../../assets/images/fields/website.png")}
              style={{
                height: 22,
                width: 22,
                resizeMode: "contain",
        ...
        React_Native_Rating_Bar.push(
          <View key={item.id} style={styles.itemHolder}>
            <Image
              source={require("../../assets/images/fields/phone.png")}
              style={{
                height: 22,
                width: 22,
                resizeMode: "contain",
            ...
            />
            <View>
              <Text style={styles.itemTitle}>رقم الجوال</Text>
              <TouchableOpacity
                onPress={() => {
                  Linking.openURL(`tel:${item[1]}`);
                }}
              >
                <Text style={styles.itemSubTitle}>{item[1]}</Text>
              </TouchableOpacity>
        ...
        React_Native_Rating_Bar.push(
          <View key={item.id} style={styles.itemHolder}>
            ...
            />
            <View>
              <Text style={styles.itemTitle}>البريد الالكتروني</Text>
              <TouchableOpacity
                onPress={() => {
                  Linking.openURL(`mailto:${item[1]}`);
                }}
              >
                <Text style={styles.itemSubTitle}>{item[1]}</Text>
              </TouchableOpacity>
        ...
        React_Native_Rating_Bar.push(
          <View key={item.id} style={styles.itemHolder}>
            <Image
              source={require("../../assets/images/fields/location.png")}
              style={{
                height: 22,
                width: 22,
                resizeMode: "contain",
            ...
            />
            <View>
              <Text style={styles.itemTitle}>العنوان</Text>
              <TouchableOpacity
                onPress={() => {
          ...
          <View style={styles.viewRatingsModalTopRatingBoxHeading}>
            <View style={{ flexDirection: "row" }}>
              <View style={{ alignItems: "flex-start" }}>
                <Text style={styles.viewRatingsModalTopRatingBoxBy}>
                  بواسطة: {item.user.name}{" "}
                </Text>
                <Text style={styles.viewRatingsModalTopRatingBoxDate}>
                  {item.created_at}
                </Text>
              </View>
            </View>
          ...
          <View style={styles.contentHolder}>
            ...
            <View style={styles.ratingHolder}>
              {this.state.ratingStars
                ? this.randerRating(this.state.ratingStars, 14)
                : this.randerRating(0, 14)}
              <Text style={styles.ratingCount}>
                ({this.state.ratingCount ? this.state.ratingCount : 0} تعليق)
              </Text>
              <TouchableOpacity
                onPress={() => {
                  this._openModal("viewRatings");
                }}
              >
            ...
            <View>
              ...
              {this.props.Login == true ? (
                <View style={styles.itemHolder}>
                  <FontAwesome5 name="flag" style={styles.itemIcon} />
                  <TouchableOpacity
                    onPress={() => {}}
                    onPress={() => {
                      this._openModal("reportOffer");
                    }}
                  >
                    <Text style={styles.itemTitle}>الإبلاغ عن الإعلان</Text>
                  </TouchableOpacity>
                </View>
              ) : null}
            </View>
            {this.props.Login == true ? (
              ...
              >
                <LinearGradient
                  ...
                  locations={[0.0, 1.0]}
                >
                  <Text style={styles.ButtonText}>اطلب العرض</Text>
                </LinearGradient>
              </TouchableOpacity>
            ) : null}
          </View>
        ...
        >
          ...
          <View style={styles.viewRatingsModalTop}>
            <View style={styles.viewRatingsModalTopRating}>
              {this.state.ratingStars
                ? this.randerRating(this.state.ratingStars, 20)
                : this.randerRating(0, 20)}
            </View>
            <Text style={styles.viewRatingsModalTopRatingCount}>
              ({this.state.ratingCount ? this.state.ratingCount : 0} تعليق)
            </Text>
          </View>
          <View style={styles.sperator2}></View>
          <ScrollView contentContainerStyle={styles.viewRatingsModalScrollView}>
            ...
            {this.props.Login == true ? (
              <LinearGradient
                style={[styles.Gradient, { width: width - 40 }]}
                colors={["#7dde9d", "#17b7bd"]}
                start={{ x: 0.0, y: 0.5 }}
                end={{ x: 1.0, y: 0.5 }}
                locations={[0.0, 1.0]}
        ...
        >
          ...
          >
            ...
            >
              <FontAwesome5
                name="times-circle"
                style={{ color: "#e0e0e0", fontSize: 20 }}
                solid
              />
            </TouchableOpacity>
            ...
            {this.state.errors.rate != "" ? (
              <Text style={styles.error}>{this.state.errors.rate}</Text>
            ) : null}
            <Text
              style={{
                textAlign: "center",
            ...
            >
              انقر علي النجوم للتقييم
            </Text>
            <View
              style={[styles.InputHolder, { borderColor: this.state.input5 }]}
            >
              <Input
                ...
                numberOfLines={4}
                onFocus={() => {
                  this.setState({ input5: "#45c9ae", input5Icon: "#45c9ae" });
                }}
                onBlur={() => {
                  this.setState({ input5: "#ebebeb", input5Icon: "#a3a3a3" });
                }}
            ...
            {this.state.errors.ratingOpinion != "" ? (
            ...
            <ScrollView contentContainerStyle={{ paddingBottom: 50 }}>
              ...
              <Form style={styles.Form}>
                ...
                <View
                  style={[
                    styles.InputHolder,
                    { borderColor: this.state.input2 }
                  ]}
                >
                  <FontAwesome5
                    name="envelope"
                    style={[styles.InputIcon, { color: this.state.input2Icon }]}
                    solid
                  />
                  <Input
                ...
                <View
                  style={[
                    styles.InputHolder,
                    { borderColor: this.state.input3 }
                  ]}
                >
                  <FontAwesome5
                    name="phone"
                    style={[styles.InputIcon, { color: this.state.input3Icon }]}
                    solid
                  />
                  <Input
                ...
                >
                  <FontAwesome5
                    ...
                    style={[
                      styles.InputIcon,
                      { color: this.state.input4Icon, marginTop: 10 }
                    ]}
                    solid
                  />
                  <Input
                ...
                {this.state.errors.content != undefined &&
                this.state.errors.content != "" ? (
                  <Text style={styles.error}>{this.state.errors.content}</Text>
                ) : null}
              </Form>
              <TouchableOpacity
                style={styles.LoginButton}
            ...
            <Form style={styles.Form}>
              ...
              >
                ...
                <Input
                  ...
                  numberOfLines={4}
                  onFocus={() => {
                    this.setState({ input4: "#45c9ae", input4Icon: "#45c9ae" });
                  }}
                  onBlur={() => {
                    this.setState({ input4: "#ebebeb", input4Icon: "#a3a3a3" });
                  }}
                  style={[
                    styles.textInput,
                    {
              ...
              {this.state.errors.reason != "" ? (
                <Text style={styles.error}>{this.state.errors.reason}</Text>
              ) : null}
            </Form>
            <TouchableOpacity
              style={styles.LoginButton}
...
const styles = StyleSheet.create({
  ...
  viewRatingsModalTopRatingCount: {
    textAlign: "center",
    color: "#aeaeae",
    fontSize: 16,
    marginTop: 10,
  ...
  viewRatingsModalTopRatingBoxHeading: {
    flexDirection: "row",
  ...
  viewRatingsModalTopRatingBoxBy: {
    textAlign: "left",
    fontSize: 12,
    color: "#a8a8a8",
    fontFamily: Constants.fontFamilyRoman
  },
  viewRatingsModalTopRatingBoxDate: {
    textAlign: "right",
    fontSize: 12,
    color: "#a8a8a8",
    fontFamily: Constants.fontFamilyRoman,
  ...
  viewRatingsModalTopRatingBoxComment: {
    textAlign: "left",
    color: "#514e5e",
    fontFamily: Constants.fontFamilyBold,
    paddingHorizontal: 20
