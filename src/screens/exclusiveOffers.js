...
import {StyleSheet, Text, View, Image, TouchableOpacity,Animated,Easing,RefreshControl, ScrollView  } from 'react-native'
...
export default class Offers extends Component {
    ...
    async getOffers() {
        try {
            ...
            const categoryApiCall = await fetch(Constants.siteUrl + 'exclusive_offers', {
    ...
    renderOffers() {
        ...
        if(this.state.offers.length > 0){
            this.state.offers.map((item, index) => {
                offersArray.push(<CardOffer key={item.id} id={item.id} Title={item.title} image={{ uri: item.image }} ratingCount={item.myrate.count} ratingStars={item.myrate.rate} offerType={item.exclusive} />)
            })
        }else{
            offersArray.push(
                <View style={{justifyContent:'center',alignItems:'center',flex:1,display:'flex'}} key={0}>
                    <FontAwesome5 name="exclamation-circle" style={{color:'red',fontSize:120,marginBottom:20,marginTop:80}} />
                    <Text style={{fontFamily: Constants.fontFamilyBold,fontSize:16}}>لايوجد عروض تناسب مدينتك</Text>
                </View>
                )
        }
    ...
    render() {
        ...
        return (
            ...
            <Container>
                <LinearGradient colors={['#7dde9d', '#17b7bd']} start={{ x: 0.0, y: 0.5 }} end={{ x: 1.0, y: 0.5 }} locations={[0.0, 1.0]} >
                    <Header style={{ backgroundColor: "transparent", display: 'flex' ,elevation:0 }}>
                        ...
                        <Body style={{ flex: 7, display: 'flex', alignItems: 'flex-start' }}>
                            <Text style={{ fontFamily: Constants.fontFamilyBold, color: '#ffffff', fontSize: 16, textAlign: 'left', writingDirection: 'ltr', }}>عروضنا الحصرية</Text>
                        </Body>
                        <Right>
                            {this.state.offers.length >0 ?
                                <TouchableOpacity style={styles.HeaderRight} onPress={() => { this._openSort() }}>
                                    <FontAwesome5 name="sort-amount-up" style={styles.HeaderRightIcon} />
                                </TouchableOpacity>
                            :null
                            }
                ...
                <Content>
                    <Animated.View style={[styles.sort,{height},this.state.isLoaded &&{padding:20}]}>
                    <TouchableOpacity style={[styles.sortTouchale, this.state.isLoaded && { display: 'flex' }]} onPress={() => { this._updateSort('latest') }}>
                            <Text style={styles.sortText}>العروض الاحدث</Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={[styles.sortTouchale, this.state.isLoaded && { display: 'flex' }]} onPress={() => { this._updateSort('oldest') }}>
...
const styles = StyleSheet.create({
    ...
    sort:{ backgroundColor: '#fff',  position: 'absolute', top: 0, right: 0, left: 0, zIndex: 5,borderBottomColor:'#ebebeb',borderBottomWidth:1},
